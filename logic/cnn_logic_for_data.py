...
def cnn_sweep(n_clusters, train, test):
    ...
    # Z-score each trace using its own mean_current stats so absolute pA level doesn't matter
    mu  = X[:, :, 0].mean(axis=1, keepdims=True)[:, :, np.newaxis]   # (n, 1, 1)
    sig = X[:, :, 0].std(axis=1,  keepdims=True)[:, :, np.newaxis] + 1e-8
    X   = (X - mu) / sig
    ...
    mu_t  = X_test[:, :, 0].mean(axis=1, keepdims=True)[:, :, np.newaxis]
    sig_t = X_test[:, :, 0].std(axis=1,  keepdims=True)[:, :, np.newaxis] + 1e-8
    X_test = (X_test - mu_t) / sig_t
...
def predict_real_event(model, feature_df, target_steps=20, normalize=True):
    feature_cols = ['mean_current', 'weighted_window_current', 'mean_minus2', 'mean_minus1', 'mean_plus1', 'mean_plus2']

    X = feature_df[feature_cols].fillna(0).values  # (n_steps, 6)

    if len(X) >= target_steps:
        X = X[:target_steps]
    else:
        pad = np.zeros((target_steps - len(X), X.shape[1]))
        X = np.vstack([X, pad])

    X = X[np.newaxis, ...]  # (1, 20, 6)

    if normalize:
        # per-trace z-score applied at training time
        mu  = X[0, :, 0].mean()
        sig = X[0, :, 0].std() + 1e-8
        X   = (X - mu) / sig

    probs = model.predict(X)[0]  # (20, n_clusters)
    pred_clusters = np.argmax(probs, axis=1) + 1  # 1-indexed to match training labels

    return pred_clusters, probs


def predict_brinkerhoff_event(model, feature_df, ios_pA=250, target_steps=20):
    """Like predict_real_event but converts I/I_os → pA before inference, no z-score."""
    feature_cols = ['mean_current', 'weighted_window_current', 'mean_minus2', 'mean_minus1', 'mean_plus1', 'mean_plus2']

    X = feature_df[feature_cols].fillna(0).values * ios_pA  # I/I_os → pA
    ...
    probs = model.predict(X)[0]  # (20, n_clusters)
    pred_clusters = np.argmax(probs, axis=1) + 1
