...
import pandas as pd
from logic.database import WINDOW_HALF, IOS
...
final_save_path = 'data/randomcontrol_feature_by_IOS.csv'
...
##  this is what i need to fix to create a feature vector that contains all the amino acids in the 5-mer window
    ...
    for sid, grp in step_groups.items():
        ...
        features.append({
            'trace_id': trace_id,
            'step_id': sid,
            'mean_current': grp['current_pA'].mean()/IOS,
            'std_current': grp['current_pA'].std()/IOS,
            ...
            'weighted_window_current': (
                ratio[0] * (step_minus2['current_pA'].mean()/IOS if step_minus2 is not None else np.nan) +
                ratio[1] * (step_minus1['current_pA'].mean()/IOS if step_minus1 is not None else np.nan) +
                ratio[2] * grp['current_pA'].mean()/IOS +
                ratio[3] * (step_plus1['current_pA'].mean()/IOS if step_plus1 is not None else np.nan) +
                ratio[4] * (step_plus2['current_pA'].mean()/IOS if step_plus2 is not None else np.nan)
            ),
            'mean_minus2': step_minus2['current_pA'].mean()/IOS if step_minus2 is not None else np.nan,
            ...
            'mean_minus1': step_minus1['current_pA'].mean()/IOS if step_minus1 is not None else np.nan,
            ...
            'mean_plus1': step_plus1['current_pA'].mean()/IOS if step_plus1 is not None else np.nan,
            ...
            'mean_plus2': step_plus2['current_pA'].mean()/IOS if step_plus2 is not None else np.nan,
