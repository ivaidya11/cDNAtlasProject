...
GAMMA          = 30  # pA — how much hydrophobicity affects current per unit (0-1) hydrophobicity
...
def plot_example_trace(database: pd.DataFrame, trace_id: int = 0):
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig('data/example_trace_with_updated_db.png', dpi=150)
    plt.close()
    print("Saved to data/example_trace_with_updated_db.png")


# ─── Main ─────────────────────────────────────────────────────────────────────

if __name__ == "__main__":
    output_path = "data/updated_nanopore_trace_database.csv"
